...
export const GameProvider: React.FC<GameProviderProps> = ({
  ...
  const handleKillPlayer = useCallback(
    (newPosition: number, currentPlayerIndex: number) => {
  ...
  const movePlayer = useCallback(
    async (playerIndex: number, roll: number) => {
    ...
    [checkSnakeOrLadder, handleKillPlayer, onGameFinish, players, triggerEvent],
  );
  ...
  }, [currentPlayer, gameStarted, movePlayer]);

  // Add more players (up to 4)
  const addPlayer = useCallback(
    (name: string, color: Player['color']) => {
      if (players.length < 4) {
